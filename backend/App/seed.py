"""
app/seed.py
Popula a base de dados com dados de exemplo para desenvolvimento e testes.
...
# ── Utilitário de hash simples (substituir por passlib em produção) ─────────
def fake_hash(password: str) -> str:
    """Hash placeholder — substituir por bcrypt/argon2 na Fase 3."""
    import hashlib
...
def seed() -> None:
    ...
    try:
        # Evitar duplicação em execuções repetidas
        if db.query(User).count() > 0:
            print("⚠️  Base de dados já contém dados. Seed ignorado.")
            return
        ...
        tech1 = User(
            name="Técnico Eliúde",
            email="eliude@helpdesk.local",
            hashed_password=fake_hash("tech123"),
            role=UserRole.tech,
        )
        tech2 = User(
            name="Técnico Davi",
            email="davi@helpdesk.local",
            hashed_password=fake_hash("tech123"),
            role=UserRole.tech,
        )
        user1 = User(
            ...
            email="eliude@helpdesk.local",
            hashed_password=fake_hash("user123"),
            role=UserRole.user,
        )
        user2 = User(
        ...
        db.add_all([admin, tech1, tech2, user1, user2])
        db.flush()
        ...
        tickets_data = [
            ...
            dict(title="VPN não conecta fora do escritório",
                 ...
                 sla_policy_id=sla_high.id,
                 sla_deadline=now - timedelta(hours=2),  # SLA já violado
                 sla_breached=True),
            ...
            dict(title="Email a reportar falha no site",
                 ...
                 source=TicketSource.email, category_id=cat_net.id,
                 creator_id=admin.id, assignee_id=tech2.id,
                 sla_policy_id=sla_critical.id,
            ...
            dict(title="Outros — dúvida geral sobre IT",
        ...
        comments = [
            Comment(ticket_id=tickets[0].id, user_id=tech1.id,
                    body="Vou verificar o cabo de alimentação e o monitor em si.", is_internal=False),
            Comment(ticket_id=tickets[0].id, user_id=tech1.id,
                    body="Nota interna: possível falha no PSU. Verificar stock de monitores.",
                    is_internal=True),
            ...
            Comment(ticket_id=tickets[2].id, user_id=user2.id,
                    body="Continua a falhar mesmo após reiniciar. Log de erro anexado.",
                    is_internal=False),
            Comment(ticket_id=tickets[5].id, user_id=tech2.id,
                    body="A investigar. Servidor de aplicação responde mas base de dados timeout.",
                    is_internal=True),
        ]
        db.add_all(comments)

        # ── 6. Histórico de exemplo ──────────────────────────────────────────
        histories = [
